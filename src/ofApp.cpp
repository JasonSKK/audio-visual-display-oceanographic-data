...
void ofApp::update(){
    ...
    if( noisea == 0){
        //bco = 22;
        bco = 22; // new for contrast 05/31/22 4:59 pm
...
void ofApp::draw(){
  ...
  //Temperature
    ...
    ofSetColor(255, 0, 0);
    ofDrawBitmapString(roundf(ofMap(osc1, 1, 1000, 18.96, 26.71) * 100) / 100, 1000, 50);
    ...
    for(int i = 0; i <= counter; i++){
        ...
        if(sign_height < osc2){
            ...
            red = ofMap(red, 1, 1000, 54, 255);
            // new mappings for contrast 05/31/22 4:59 pm
            sleep(0.1);
        };
        if(sign_height > osc2){
            sign_height = sign_height - 0.0125;
            //red = ofMap(sign_height, 1, 1000, 55, 255);
            red = ofMap(sign_height, 1, 1000, 54, 255);
            // new mapping for contrast 05/31/22 4:59 pm
        ...
        if(z_upcrossing < osc3){
            ...
            green = ofMap(z_upcrossing, 1, 1000, 43, 200);
            // new mappings for contrast 05/31/22 4:59 pm
            sleep(0.1);
        };
        if(z_upcrossing > osc3){
